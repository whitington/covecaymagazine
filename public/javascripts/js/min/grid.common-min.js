...
{case"textarea":elem=document.createElement("textarea");if(!options.cols&&elm){jQuery(elem).css("width","99%");}
jQuery(elem).attr(options);jQuery(elem).val(vl);break;case"checkbox":elem=document.createElement("input");elem.type="checkbox";jQuery(elem).attr({id:options.id,name:options.name});if(!options.value){if(vl.toLowerCase()=='on'){elem.checked=true;elem.defaultChecked=true;elem.value=vl;}else{elem.value="on";}
...
break;case"text":elem=document.createElement("input");elem.type="text";elem.value=vl;if(!options.size&&elm){jQuery(elem).css("width","99%");}
jQuery(elem).attr(options);break;case"password":elem=document.createElement("input");elem.type="password";elem.value=vl;if(!options.size&&elm){jQuery(elem).css("width","99%");}
